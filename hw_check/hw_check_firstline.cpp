...
FILE_STATUS getaprint_firfilestatus(const string& filepath, const string& filename, const Student& student)
{
	...
	if (!containsNewline(fin))
	{
		cout << FILESTATUS_STR[NOTWIN] << endl;
		fin.close();
		return NOTWIN;
	}
	fin.clear();
	...
	// 处理专业简称中的 | 分隔符
	size_t delimiter_pos = student.majorshortname.find('|');
	if (delimiter_pos != string::npos) {
		// 如果有 |，则检查 | 前后的部分是否出现在 comitem 中
		string part1 = student.majorshortname.substr(0, delimiter_pos);
		string part2 = student.majorshortname.substr(delimiter_pos + 1);
		if (find(comitem.begin(), comitem.end(), part1) != comitem.end() ||
			find(comitem.begin(), comitem.end(), part2) != comitem.end()) {
			have_major = 1;
		}
	}
	else {
